...
const ensureDeps = () =>
    Promise.all(
        DEPS.map((dep) =>
            root[dep.global] ? Promise.resolve() : loadScript(dep.url)
        )
    ).then(() => {
...
function wireBehavior(overlay) {
    ...
    let isMuted = false;
    const applyMute = () => {
        overlay.querySelectorAll("video").forEach((v) => { v.muted = isMuted; });
    ...
    overlay.addEventListener("click", (e) => {
        ...
        if (e.target.closest("[data-short-video-share]")) {
            ...
            const slide = shareBtn.closest(".swiper-slide") || playerEl?.querySelector(".swiper-slide-active");
            const v = slide?.querySelector("video") || getActiveVideo();
            const title = slide?.querySelector(".short-video-stage__title p")?.textContent?.trim() || doc.title;
    ...
    playerEl.querySelectorAll(".swiper-slide").forEach((slide) => {
        ...
        v.addEventListener("loadedmetadata", () => { setProgress(0); updateTime(); });
    ...
    document.addEventListener("keydown", (e) => {
        if (!isOpen()) return;
        const inst = getInst();
        if (e.key === "ArrowUp") { e.preventDefault(); inst?.slidePrev(); }
        else if (e.key === "ArrowDown") { e.preventDefault(); inst?.slideNext(); }
