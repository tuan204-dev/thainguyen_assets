                ...
                scriptEl.addEventListener(
                    ...
                    () => {
                        ...
                        if (loadedSwiper) {
                            ...
                            reject(
                                new Error("Swiper script loaded but window.Swiper is unavailable."),
                            );
                        }
                    },
                    { once: true },
        ...
        function ensureSwiperLoaded() {
            ...
            const tryLoad = (index) => {
                if (index >= SWIPER_SCRIPT_URLS.length) {
                    return Promise.reject(new Error("All Swiper script URLs failed to load."));
                }

                return loadScript(SWIPER_SCRIPT_URLS[index]).catch(() => tryLoad(index + 1));
            };
