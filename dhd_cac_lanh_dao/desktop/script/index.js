const projects = [
    [
        "ct-cum-cong-nghiep-hanh-phuc.png",
        "Cụm công nghiệp Hạnh Phúc - Xuân Phương",
        "09:37, 19/09/2025",
    ],
    [
        "ct-khu-du-lich-ho-nui-coc.png",
    ...
    [
        "ct-san-gon-glory-ho-suoi-lanh.png",
        "Sân gôn Glory tại khu vực hồ Suối Lạnh, xã Thành Công",
        "15:33, 17/09/2025",
    ],
    [
        "ct-khu-van-hoa-van-xuan.jpg",
        "Khu văn hóa, thể thao, công viên cây xanh phường Vạn Xuân",
        "10:57, 16/09/2025",
    ],
    [
        "ct-benh-vien-a.jpg",
        "Trung tâm Phụ - Sản - Phẫu thuật gây mê hồi sức và thiết bị y tế, Bệnh viện A Thái Nguyên",
        "16:22, 15/09/2025",
    ],
    [
        "ct-duong-quang-khe-khang-ninh.jpg",
        "Dự án xây dựng Tuyến đường Quảng Khê - Khang Ninh",
        "17:45, 29/08/2025",
    ],
    [
        "ct-trung-tam-y-te-ngan-son.jpg",
        "Dự án xây dựng Trung tâm Y tế Ngân Sơn",
        "17:39, 29/08/2025",
    ],
    [
        "ct-den-muc-chua-huong-ap.jpg",
        "Tu bộ, tôn tạo đền Mục và chùa Hương Ấp",
        "14:18, 06/03/2025",
    ],
    [
        "ct-tru-so-lam-viec-khoi-co-quan.jpg",
    ...
    [
        "ct-duong-vanh-dai-v.jpg",
        "Đường Vành đai V đoạn qua khu vực tỉnh Thái Nguyên",
        "14:03, 06/03/2025",
    ],
...
function renderProjects() {
    ...
    document.getElementById("projectTopGrid").innerHTML = projects
        .slice(1, 4)
        .map(
            ([img, title, date]) => `
    ...
    <article class="t:flex t:gap-5 t:border-b t:border-[#eee] t:py-5">
      ...
      <div><h2 class="t:text-lg t:font-bold t:leading-snug">${title}</h2><p class="t:mt-1 t:text-sm t:text-[#777]">${date}</p><p class="t:mt-2 t:text-sm t:leading-6 t:text-[#555]">Công trình trọng điểm chào mừng Đại hội Đảng bộ tỉnh Thái Nguyên nhiệm kỳ 2025-2030.</p></div>
            ...
            .join("") +
        `<button type="button" class="t:mx-auto t:mt-6 t:border-0 t:bg-transparent t:text-[#f71917]">Xem thêm</button>`;
    document.getElementById("sideNews").innerHTML = projects
        .slice(0, 3)
        .map(
            ([img, title, date]) => `
...
function setTab(tab, shouldScroll = true) {
    ...
    if (tab === "standing") {
        ...
        if (shouldScroll) {
            document
                .getElementById("standingSection")
                .scrollIntoView({ behavior: "smooth", block: "start" });
        }
    }
    if (tab === "committee") {
        ...
        if (shouldScroll) {
            document
                .getElementById("committeeSection")
                .scrollIntoView({ behavior: "smooth", block: "start" });
        }
    }
    if (tab === "projects") {
